with
    report as (select date('{{ start_date }}') as t1, date('{{ end_date }}') as t2),
    drop_reinstated_cancellations as (
        -- This is a requirement from CGICE, reinstatements and cancellations adding
        -- up to
            ...
            case
                when
                    (
                        ...
                        and lag(transaction_type) over (
                            partition by policy_number order by transaction_at
                        )
                        = 'Cancel'
                        and gross_premium_ipt_inc + lag(gross_premium_ipt_inc) over (
                            partition by policy_number order by transaction_at
                        )
                        = 0
                    )
                    or (
                        ...
                        and lead(transaction_type) over (
                            partition by policy_number order by transaction_at
                        )
                        = 'Reinstatement'
                        and gross_premium_ipt_inc + lead(gross_premium_ipt_inc) over (
                            partition by policy_number order by transaction_at
                        )
                        = 0
                    )
        ...
        from dbt_marts.cgice_cumulative_premium_bdx, report
...
select
    ...
    format(
        '%.2f', net_rated_premium_due_to_underwriter
    ) as `Net Rated Premium Due To Underwriter IPT exc`,
