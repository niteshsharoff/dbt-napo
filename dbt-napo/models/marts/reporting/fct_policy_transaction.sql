...
with
  ...
  , with_discount_and_ipt_amount as (
    select *
      ...
      , retail_price - {{target.schema}}.calculate_amount_exc_ipt(retail_price) as retail_ipt_amount
      , premium_price - {{target.schema}}.calculate_amount_exc_ipt(premium_price) as premium_ipt_amount
    from with_premium_and_retail_price
  )
  , apply_rounding as (
    -- pre-mature rounding to keep reporting in line with premium_bdx
    select * except(retail_price, premium_price, discount_amount, retail_ipt_amount, premium_ipt_amount)
      ...
      , round(cast(retail_ipt_amount as numeric), 2, "ROUND_HALF_EVEN") as retail_ipt_amount
      , round(cast(premium_ipt_amount as numeric), 2, "ROUND_HALF_EVEN") as premium_ipt_amount
  ...
  , positions as (
    ...
    select *
      , case 
        ...
        then {{target.schema}}.calculate_consumed_amount(retail_price, policy.start_date, policy.end_date, policy.cancel_date)
      ...
      , case
        when transaction_type = 'Cancellation' 
          or transaction_type = 'Cancellation MTA'
        then {{target.schema}}.calculate_consumed_amount(retail_ipt_amount, policy.start_date, policy.end_date, policy.cancel_date)
        when transaction_type = 'NTU'
        then 0.0
      ...
      , case
        when transaction_type = 'Cancellation' 
          or transaction_type = 'Cancellation MTA'
        then {{target.schema}}.calculate_consumed_amount(premium_ipt_amount, policy.start_date, policy.end_date, policy.cancel_date)
        when transaction_type = 'NTU'
        then 0.0
  ...
  , with_underwriter_dimension as (
    select
      ...
      , struct(
        ...
        , cast(retail_position - retail_ipt_position as numeric) as premium_position_ipt_exc
        , cast(retail_ipt_position as numeric) as ipt_position
        , cast(retail_difference as numeric) as premium_difference_ipt_inc
        , cast(retail_difference - retail_ipt_difference as numeric) as premium_difference_ipt_exc
      ...
      , struct(
        ...
        , cast(premium_position - premium_ipt_position as numeric) as premium_position_ipt_exc
        ...
        , cast(premium_difference - premium_ipt_difference as numeric) as premium_difference_ipt_exc
