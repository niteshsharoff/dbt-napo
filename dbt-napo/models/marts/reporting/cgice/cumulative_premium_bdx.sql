...
, cgice_transactions as (
  select transaction_type
    ...
    , policy.quote_source
    , format('%.2f', underwriter.discount_difference) as discount_amount
    , trim(customer.first_name) || ' ' || trim(customer.last_name) as customer_name
    ...
    , case
      when pet.breed_category = 'mixed' then 'mixed'
      when pet.breed_category = 'pedigree' and pet.breed_name is null then 'unknown'
      when pet.breed_category = 'pedigree' and pet.breed_name is not null then pet.breed_name
      when pet.breed_category = 'cross' and pet.breed_name is null then 'cross'
      when pet.breed_category = 'cross' and pet.breed_name is not null then pet.breed_name ||  ' + cross'
    end as pet_breed
    , dbt_jeremiahmai.calculate_age_in_months(pet.date_of_birth, policy.start_date) as pet_age
    , pet.gender as pet_gender
    , format('%.2f', pet.cost) as pet_cost
    , pet.is_microchipped as pet_chipped
    , pet.is_neutered as pet_neutered
    , case
      when pet.multipet_number is null then False
      else True
    end as multipet
    ...
    , underwriter.premium_difference_ipt_inc - underwriter.ipt_difference as gross_premium_ipt_exc
    ...
    , 0.65 * underwriter.premium_difference_ipt_exc as net_rated_premium_due_to_underwriter
    , 0.65 * underwriter.premium_difference_ipt_exc + underwriter.ipt_difference as total_due_to_underwriter
...
, unchanged_transactions as (
  ...
  where transaction_type = 'New Policy' 
...
, all_transactions as (
  select * from unchanged_transactions
  union all
...
select * except(
  ...
  , round(net_rated_premium_due_to_underwriter, 2, "ROUND_HALF_EVEN") as net_rated_premium_due_to_underwriter
