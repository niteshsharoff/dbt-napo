    ...
    extended_ledger as (
        select
            * except (sold, prev_sold, cancel, prev_cancel),
            ...
            sum(
                case when cancel = true and prev_cancel = false then 1 else 0 end
            ) over (partition by policy_id order by change_at) as _cancel_grp,
            sum(
                case when cancel = false and prev_cancel = true then 1 else 0 end
            ) over (partition by policy_id order by change_at) as _reinstate_grp
        from
            (
                select
                    *,
                    {{ target.schema }}.is_sold(
                        annual_payment_id, subscription_id
                    ) as sold,
                    lead(
                        {{ target.schema }}.is_sold(annual_payment_id, subscription_id)
                    ) over (partition by policy_id order by change_at desc)
                    as prev_sold,
                    {{ target.schema }}.is_cancelled(
                        annual_payment_id, subscription_id, cancel_date
                    ) as cancel,
                    lead(
                        {{ target.schema }}.is_cancelled(
                            annual_payment_id, subscription_id, cancel_date
    ...
    policy_ledger as (
        select
            * except (
            ...
            extract(
                date from timestamp_millis(accident_cover_start_date)
            ) as accident_cover_start_date,
            extract(
                date from timestamp_millis(illness_cover_start_date)
            ...
            ) over (partition by policy_id, _cancel_grp order by change_at)
            as cancelled_at,
            ...
            ) over (partition by policy_id, _reinstate_grp order by change_at)
            as reinstated_at,
            timestamp_millis(effective_at) as effective_from,
            lead(
                timestamp_millis(effective_at), 1, timestamp("2999-01-01 00:00:00+00")
            ) over (partition by policy_id order by effective_at) as effective_to,
            max(version_id) over (
                partition by policy_id order by version_id desc
            ) as latest_version
