...
select
    ...
    ,case 
      ...
      ,sum(cast((select json_extract_scalar(c,'$.value') from unnest(json_extract_array(conversions,'$')) c where json_extract_scalar(c,'$.action_type')='offsite_conversion.fb_pixel_custom.academy_registration_success') as numeric)) as academy_registration_success_conversions
      ,sum(cast((select json_extract_scalar(c,'$.value') from unnest(json_extract_array(conversions,'$')) c where json_extract_scalar(c,'$.action_type')='offsite_conversion.fb_pixel_custom.start_academy_registration') as numeric)) as start_academy_registration_conversions 
