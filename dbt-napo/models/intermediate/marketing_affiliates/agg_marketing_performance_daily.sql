...
with
    fb as (select * from {{ ref("agg_marketing_facebook_ads") }}),

    bing as (select * from {{ ref("agg_marketing_bing_ads") }}),

    gads as (select * from {{ ref("agg_marketing_google_ads") }}),
    ...
    date_spine as (
        ...
        from
            unnest(
                generate_date_array(
                    '2023-01-01', date_sub(current_date(), interval 1 day)
                )
            ) dt
    ),
    ...
    joined as (
        select
            ...
            round(
                sum(b.conversions) + sum(c.conversions) + sum(d.conversions), 3
            ) as conversions,
