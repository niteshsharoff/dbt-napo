with
    ...
    subscription_table as (  -- latest subscription table
        select * except (row_no) from grouped_policy_data where row_no = 1
    ),
    active_policy_existed as (
        select policy_id, countif(active = true) as active_subscription_existed
    ...
    grouped_data as (
        select
            ...
            -- ,coalesce(if(c.active_subscription_existed=1,true,false),false) as
            -- active_subscription_existed
        ...
        left join
            subscription_table s  -- only has last subscription
            on p.policy_id = s.policy_id
        left join active_policy_existed c on c.policy_id = p.policy_id
    ),
    policy_data_with_payment_data as (
        select
            a.*,
            first_value(b.charge_date) over (
                partition by policy_id
                order by charge_date asc
                rows between unbounded preceding and unbounded following
            ) as first_payment_charge_date,
            last_value(b.charge_date) over (
                partition by policy_id
                order by charge_date asc
                rows between unbounded preceding and unbounded following
            ) as last_payment_charge_date
        from grouped_data a
        left join {{ ref("int_payments") }} b using (policy_id)
