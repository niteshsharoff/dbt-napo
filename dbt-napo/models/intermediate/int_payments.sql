with payments as (
select *
from `ae32-vpcservice-datawarehouse`.`dbt_nsharoff`.`stg_policy_payments`
),
subscriptions as (
  select distinct 
         subscription_id
        ,policy_id 
from `ae32-vpcservice-datawarehouse`.`dbt_nsharoff`.`stg_subscription`
),
payments_monthly as (
select a.*
      ,b.policy_id
...
from `ae32-vpcservice-datawarehouse.dbt_nsharoff.stg_policy` b
