with
    ...
    first_time_cancellations as (
        select
            'Cancel Policy' as transaction_type, row_effective_from as transaction_at, *
        from policy_history
    ...
    all_transactions as (
        ...
        union all
        select *
        from first_time_cancellations
    )
select
    transaction_type, transaction_at, quote, policy, product, customer, user, pet, breed
