...
*/
with cumulative_paid_amount_by_cover_snapshot as (
  select policy_uuid
    , claim_source
    , claim_cover_type
...
pivot_paid_amount_to_policy_grain as (
  select *
  from cumulative_paid_amount_by_cover_snapshot
  pivot(sum(paid_amount) for claim_cover_type in (
    'vet_fee_cover' as used_vet_fee_cover
    , 'complementary_treatment_cover' as used_complementary_treatment_cover
...
, final as (
  select c.policy_reference_number
