...
*/
select *, date_diff(snapshot_date, prev_snapshot_date, day) as diff
from
    (
        select
            ...
            lag(snapshot_date) over (
                partition by claim_id order by snapshot_date
            ) as prev_snapshot_date
        from {{ ref("int_underwriter__claim_snapshot") }}
        order by snapshot_date
    )
where
    date_diff(snapshot_date, prev_snapshot_date, day) != 1
